package org.opentmf.resource.model;

import com.fasterxml.jackson.annotation.JsonTypeInfo;
import jakarta.validation.Valid;
import lombok.Getter;
import lombok.Setter;
import org.opentmf.common.model.CharacteristicRelationship;
import org.opentmf.common.model.ResourceGraphRef;

/**
 * Describes links between resource graphs.
 *
 * <p><br/>
 * <strong>Referring TMF artifacts:</strong>
 * <ul>
 *   <li>TMF-664: Resource Function Activation Management API</li>
 *   <li>TMF-730: Software And Compute Management API</li>
 * </ul>
 * </p>
 *
 * @author Gökhan Demir
 */
@Getter
@Setter
@JsonTypeInfo(
    use = JsonTypeInfo.Id.NAME,
    visible = true,
    include = JsonTypeInfo.As.EXISTING_PROPERTY,
    defaultImpl = ResourceGraphRelationship.class
)
public class ResourceGraphRelationship extends CharacteristicRelationship implements IResourceGraphRelationship {

  /**
   * Resource graph reference.
   */
  private @Valid ResourceGraphRef resourceGraph;
}